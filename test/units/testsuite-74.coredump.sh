...
mkdir -p /run/systemd/system/systemd-coredumpd@.service.d/
cat >/run/systemd/system/systemd-coredumpd@.service.d/99-allow-namespace-change.conf <<EOF
[Service]
Environment=SYSTEMD_COREDUMP_ALLOW_NAMESPACE_CHANGE=1
EOF
systemctl daemon-reload
run_namespaced_coredump_tests
